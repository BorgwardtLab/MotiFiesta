...
for m_type in ['barbell', 'star', 'random', 'clique']:
    for d in [0, .01, .02, .05, .1, .2]:
        SyntheticMotifs(root=f'synth-distort-{m_type}-d{d:.2f}', seed=42, motif_size=10, motif_type=m_type, parent_size=20, distort_p=d)

# Multi-motif
print(">>> MULTI MOTIF")
for n_motifs in [3, 5, 10]:
    for d in [0, .01, .02, .05, .1, .2]:
        SyntheticMotifs(root=f'synth-{n_motifs}motifs-d{d:.2f}', seed=42, motif_size=6, parent_size=(6* n_motifs * 2), distort_p=d, n_motifs=n_motifs)

# size
print(f">>> SIZE")
for motif_size in [5, 10, 20, 30, 40]:
    for d in [0, .01, .02, .05, .1, .2]:
        SyntheticMotifs(root=f'synth-{motif_size}nodes-d{d:.2f}', motif_type='random', seed=42, motif_size=motif_size, parent_size=motif_size*2, distort_p=d)

# density
print(">>> SPARSITY")
for sparsity in [.3, .5, 1]:
    for d in [0, .01, .02, .05, .1, .2]:
        SyntheticMotifs(root=f'synth-barbell-s{sparsity:.2f}', seed=42, motif_size=10, parent_size=20, distort_p=d, motif_type='random')
